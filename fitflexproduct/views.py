        ...
        if 'category' in request.GET:
            ...
            if category_friendly_name:
                selected_category = get_object_or_404(
                    Category, friendly_name__iexact=category_friendly_name)
                products = products.filter(category=selected_category)

        if 'difficulty' in request.GET:
            difficulty_name = request.GET.get('difficulty')
            if difficulty_name:
                selected_difficulty = get_object_or_404(
                    DifficultyLevel, name__iexact=difficulty_name)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('product_detail',
                            product_id=review.workout_program.id)
        else:
